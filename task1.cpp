...
// Класс SemaphoreSlim реализует семафоры на основе condition_variable и mutex.
// Используется для более легковесного контроля доступа, но может быть менее производительным.
class SemaphoreSlim {
public:
    ...
    void acquire() {
        ...
        // Ожидание, пока счетчик не станет больше нуля.
        cv.wait(lock, [this]() { return iCount > 0; });
        --iCount; // Уменьшение счетчика.
    }

    void release() {
        lock_guard<mutex> lock(mtx);
        if (iCount < mCount) {
            ++iCount; // Увеличение счетчика.
            cv.notify_one(); // Уведомление одного ожидающего потока.
        }
    }

private:
    mutex mtx; // Мьютекс для защиты доступа к счетчику.
...
// Потоковая функция с использованием SemaphoreSlim для синхронизации.
...
int main() {
    ...
    // Тестирование с использованием SemaphoreSlim.
